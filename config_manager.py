...
def test_llm_config(interface_format, api_key, base_url, model_name, temperature, max_tokens, timeout, log_func, handle_exception_func):
    ...
    def task():
        try:
            log_func("开始测试LLM配置...")
            log_func(f"接口: {interface_format}, 模型: {model_name}")
            ...
            log_func(f"适配器: {type(llm_adapter).__name__}")
            ...
            if response:
                ...
                log_func("请检查API密钥、Base URL和模型名称是否正确")
...
def test_embedding_config(api_key, base_url, interface_format, model_name, log_func, handle_exception_func):
    ...
    def task():
        try:
            ...
            log_func(f"接口: {interface_format}, 模型: {model_name}, Base URL: {base_url}")
            ...
            log_func(f"适配器: {type(embedding_adapter).__name__}")

            test_text = "测试文本"
            log_func(f"正在调用Embedding API...")
            embeddings = embedding_adapter.embed_query(test_text)
            log_func(f"Embedding返回结果: {embeddings}")
